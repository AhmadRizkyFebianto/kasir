...
import { createPayment as createMidtransPayment } from '@/lib/payment/midtrans-client';
import { createPaymentTransaction, updatePaymentStatus } from '@/lib/reservations/payment-queries';
...
export async function createOrder(formData: FormData) {
  ...
  try {
    ...
    if (!['cash', 'qris', 'bank_transfer', 'e_wallet', 'midtrans'].includes(method)) return { error: 'Metode pembayaran tidak valid' };
    ...
    const { error: paymentError } = await supabase.from('payments').insert({
      ...
      midtrans_token: method === 'midtrans' ? (await createMidtransPayment({
        reservationId: order.id,
        amount: totalAmount,
        customer: {
          firstName: user.email || 'Customer',
...
export async function createMidtransPaymentAction({
  reservationId,
  amount,
  customer,
}: {
  ...
  try {
    ...
    return { success: true, token: result.token, redirectUrl: result.redirectUrl };
  } catch (error) {
    console.error('Midtrans payment error:', error);
    return { success: false, error: 'Failed to create Midtrans payment' };
  }
}
